---
title: "Groundwater Quality - Machine Learning Categories"
...
# load data file 
master <- read_csv("NO3_precip_temp_region.csv")
dim(master)
...
# set categories based on nitrate concentration
commast <- commast %>%
  mutate(nitrate = case_when(nitrate != 0 ~ "1", TRUE ~ "0")) # detected nitrate is considered contaminated for this model
...
rf_grid <- expand.grid(mtry = c(20, 25, 30, 35, 40, 45), trees = c(500, 1000, 1500, 2000, 2500))
...
rf_tune_results <- rf_workflow %>%
  tune_grid(resamples = commast_cv, #CV object
            ...
            metrics = metric_set(accuracy, kap) # model metrics we care about
  )
...
autoplot(rf_tune_results, metric = "accuracy") # plot results with accuracy
autoplot(rf_tune_results, metric = "kap") # plot results with Cohen's kappa

param_final <- rf_tune_results %>%
  # select model based on best Cohen's kappa
